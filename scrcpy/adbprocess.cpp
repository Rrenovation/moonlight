...
bool Adbprocess::appprocess()
{
    ...
    if (readAllStandardOutput().contains("DesktopSetDevice"))
    {
        return true;
    }
    return false;
...
bool Adbprocess::autoConnect(int port)
{
    // UNCONNECT,          //adb未连接
    ...
    case UNCONNECT:
        ...
        break;
    case WAITPUSH:
        if (push("lewone-server", "/data/local/tmp/lewone-server"))
        {
